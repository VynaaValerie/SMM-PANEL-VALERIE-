...
export default async function (m, conn = { user: {} }) {
  ...
  // Format rapi untuk pesan
  console.log(`
┏━━• ${chalk.redBright(me + ' ~ ' + conn.user.name)} ❳
┃❴ 𝐓𝐢𝐦𝐞 ❵
┃⏰ ${chalk.black(chalk.bgYellow((m.messageTimestamp ? new Date(1000 * (m.messageTimestamp.low || m.messageTimestamp)) : new Date).toLocaleString('id', { timeZone: 'Asia/Jakarta' })))}
┃❴ 𝐌𝐞𝐬𝐬𝐚𝐠𝐞 ❵
┃📑 ${chalk.magenta(m.messageStubType ? WAMessageStubType[m.messageStubType] : '')} [${filesize} ${filesize === 0 ? 'B' : (filesize / 1009 ** Math.floor(Math.log(filesize) / Math.log(1000))).toFixed(1)}${['', ...'KMGTP'][Math.floor(Math.log(filesize) / Math.log(1000))] || ''}]
┃❴ 𝐒𝐢𝐳𝐞 ❵
┃📊 ${filesize} bytes
┃❴ 𝐅𝐫𝐨𝐦 ❵
┃📤 ${chalk.green(sender)}
...
`.trim());
  ...
  // Memproses dan menampilkan teks
